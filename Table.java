...
public class Table implements Serializable {
    ...
    public Object getUpdatedPrimary(Hashtable<String, Object> htblColNameValue) {
    	return null; //TODO
    }
    public String searchForKey(Object o,int type) {
    	return "";//TODO
    }
    public Hashtable<String,Object> hashTheTuple(Tuple t) {
	...
	public static int whereIsIt(Object o, String min, String max, String before, String after) {
		...
		if (oo.compareTo(min) >= 0 && oo.compareTo(max) <= 0 || before.equals("this is the first page")
	...
	public void insertSorted(Tuple x, Object keyValue) {
		...
		while (true) {
			// kont mafrod a7ot el condition dah gowa el while ana 3aref bas mesh 3aref
			// 3amalt keda leh we mekasel ashofha
			if (lower < upper) {
	...
	public boolean invalidUpdate(String strKey, Hashtable<String, Object> htblColNameValue,
			Vector<String[]> metaOfTable) throws Exception {
	...
	public void updateTable(String strKey, int type, Hashtable<String, Object> htblColNameValue,
			Vector<String[]> metaOfTable) throws Exception {

		if (invalidUpdate(strKey, htblColNameValue, metaOfTable)) {
			System.out.println("Invalid update ya khawal t3btny, khod elreturn elmateen");
			return;
		}
		Object x=null;
		try {
		switch (type) {
		case 1:
			x = Integer.parseInt(strKey);
			break;
		case 2:
			x = strKey;
			break;
		default:
			x = Double.parseDouble(strKey);
			break;
		}}catch(Exception e) {
			e.printStackTrace();
		}
       String pagename= searchForKey(x,type);		//TODO
       if(pagename.equals("not found")) {
    	   System.out.println("not found");
    	   return;
       }
       Page p=deserialize(pagename);
       Tuple newTup= p.updateDesiredTuple(strKey,htblColNameValue); //TODO
       Object y=getUpdatedPrimary(htblColNameValue);
       if(y!=null) {
       if(!searchForKey(y,type).equals("not found")) {
    	   System.out.println("primary key already there");
       }else {
    	   Hashtable<String,Object> entryAsHash=hashTheTuple(newTup);
    	   DBApp db=new DBApp();
    	   db.insertIntoTable(getTableName(),entryAsHash);
    	   db.deleteFromTable(getTableName(),entryAsHash);
       }
       }
