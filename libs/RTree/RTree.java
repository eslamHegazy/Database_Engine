...
public class RTree<T> implements TreeIndex{
	...
	public List<T> search(Polygons p) {
		double[] coords = new double[2];
		double[] dimensions = new double[2];
		convertPolygonToArr(p, coords, dimensions);
	...
	public boolean delete(Polygons p) {
		double[] coords = new double[2];
		double[] dimensions = new double[2];
		convertPolygonToArr(p, coords, dimensions);
		return delete(coords, dimensions,(T) p);
	}
	/**
	...
	private Node[] splitNode(Node n) {
		...
		while (!cc.isEmpty()) {
			if ((nn[0].children.size() >= minEntries) && (nn[1].children.size() + cc.size() == minEntries)) {
				...
				cc.clear();
				return nn;
			} else if ((nn[1].children.size() >= minEntries) && (nn[1].children.size() + cc.size() == minEntries)) {
	...
	private class Node {
		...
		public String toString() {
			...
			for (int i = 0; i < coords.length; i++)
				sb.append(String.format("(%.1f,%.1f)", coords[i], dimensions[i]));
			sb.append("\nChildren: ");
			int i = 0;
			for (Node e : children) {
				sb.append("ch" + (i++) + ":" + e);
	...
	private class Entry extends Node {
		...
		public String toString() {
			StringBuilder sb = new StringBuilder();
			// for 2dimensions
			sb.append(String.format("(%.1f,%.1f),(%.1f,%.1f)->%s\t", coords[0], dimensions[0], coords[1], dimensions[1],
					entry));
			return sb.toString();
		}
	}
	...
	public void convertPolygonToArr(Polygons p,double[] coords, double[] dimensions) {
		Rectangle rec = p.getBounds();
		double x = rec.getX(); double y=rec.getY();
		double width = rec.getWidth(); double height = rec.getHeight();
		coords = new double[] {x,y};
		dimensions = new double[] {width,height};
	}
}
