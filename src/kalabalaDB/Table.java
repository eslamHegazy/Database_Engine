...
public class Table implements Serializable {
	...
	public Iterator<Tuple> selectFromTable(SQLTerm[] arrSQLTerms, String[] strarrOperators,
		...
		Iterator<Tuple> s = inspectCols(arrSQLTerms, strarrOperators, metaOfTable);// method inspect cols determines
																					// which way to search through pages
		if (s != null)
			return s;
	...
	public Iterator<Tuple> inspectCols(SQLTerm[] arrSQLTerms, String[] strarrOperators, Vector<String[]> metaOfTable)
			throws DBAppException {
		// check for colnames or validity type .. returns true if clustering key has an
		// index
	...
	/*
	 * public void seeAnother(Object keyValue, String pageName, int newPPos) { try {
	 * while (newPPos < pages.size()) { Page p = deserialize(pageName);
	 * FileInputStream fileIn = new FileInputStream(pageName + ".ser");
	 * ObjectInputStream in = new ObjectInputStream(fileIn); Page xx = (Page)
	 * in.readObject(); in.close(); fileIn.close(); p.deleteInPage2(keyValue,
	 * primaryPos); if (p.size() == 0) { File f = new File(pageName + ".ser");
	 * f.delete(); for (int i = 0; i < pages.size(); i++) { if
	 * (pages.get(i).equals(pageName)) { MinMax.remove(i); pages.remove(i); } } }
	 * else { Tuple t0 = p.getTuples().get(0); Tuple tn = p.getTuples().get(p.size()
	 * - 1); String k0 = (t0.getAttributes().get(primaryPos)).toString(); String kn
	 * = (tn.getAttributes().get(primaryPos)).toString(); String minMax = k0 + "," +
	 * kn; for (int i = 0; i < pages.size(); i++) { if
	 * (pages.get(i).equals(pageName)) { MinMax.setElementAt(minMax, i); } } }
	 * newPPos++; } } catch (ClassNotFoundException | IOException e) { // TODO
	...
	// this.colNameBTreeIndex =
	// ((HashtableSerializer)(in.readObject())).getHashtable();
	// }
