...
public class Table implements Serializable {
	...
	/*
	 * public Vector<Object> getMin() { return min; }
	 * 
	 * public void setMin(Vector<Object> min) { this.min = min; }
	 * 
	 * public Vector<Object> getMax() { return max; }
	 * 
	 * public void setMax(Vector<Object> max) { this.max = max; }
	 */
	public void setStrClusteringKey(String strClusteringKey) {
		this.strClusteringKey = strClusteringKey;
	}
	...
	public Comparable getMax(int index) throws DBAppException
	{
		String pageName = pages.get(index);
		Page p = deserialize(pageName);
		Comparable maxx = (Comparable)p.getTuples().get(p.size() - 1).getAttributes().get(primaryPos);
		
		return maxx;
	}

	public Hashtable<Tuple,String> addInPage(int curr, Tuple x, String keyType, String keyColName, int nodeSize,boolean doInsert,Hashtable<Tuple,String>list) throws DBAppException, IOException {
		// System.out.println(x+" "+curr);\
...
//		}
	...
	private Ref getClusterReference(Object keyV,String keyColName) throws DBAppException {
		...
		if(colNameTreeIndex.contains(keyColName)){
			...
			for(int i=pages.size()-1;i>=0;i--){
				// TODO get the min max at position i
	...
	public void deleteInTable(Hashtable<String, Object> htblColNameValue, Vector<String[]> metaOfTable,
		...
		if (!(indicesGiven.size() == 0)) {
			...
			for (int i = 0; i < pages.size(); i++) {
				...
				if (((Comparable) htblColNameValue.get(strClusteringKey)).compareTo(((Comparable) getMin(i))) < 0) {//TODO get the min 
...
//	}
	...
	private ArrayList<Tuple> ltOrLtlLinear(String _strColumnName, Object _objValue, String _strOperator,int pos) throws DBAppException  {
		...
		ArrayList<Tuple> res=new ArrayList();
		for(int i=0;i<pages.size();i++) {
			if(((Comparable) getMin(i)).compareTo((Comparable)_objValue)>0)break;
			if(((Comparable) getMin(i)).compareTo((Comparable)_objValue)==0&&_strOperator.length()==1)break;
	...
	//	System.out.println("p"+pages.size());
		...
		for(int i=pages.size()-1;i>=0;i--) {
			System.out.println(i);
			if(((Comparable) getMax(i)).compareTo((Comparable)_objValue)<0)break;
			if(((Comparable) getMin(i)).compareTo((Comparable)_objValue)==0&&_strOperator.length()==1)break;
			Page x=deserialize(pages.get(i));
			int j=x.getTuples().size()-1;
			while(j>=0&&((Comparable)x.getTuples().get(j).getAttributes().get(pos)).compareTo((Comparable)_objValue)>0) {
	...
	private ArrayList<Tuple> equalsBinary(String _strColumnName, Object _objValue, String _strOperator, int pos) throws DBAppException{
		...
		for (int pageIdx = startPageIndex ,tupleIdx = startTupleIndex; pageIdx<pages.size(); pageIdx++,tupleIdx=0) {
			...
			if(((Comparable) getMin(pageIdx)).compareTo((Comparable)_objValue)>0)break;// TODO : get min
...
//	 
	...
	public String toString() {
		 ...
		 * sb.append(min.get(i)+", "); } if (min.size()>0)
		 * sb.append(min.get(min.size()-1)+"}\n");
		 * 
		 * sb.append("Max:\n{"); for (int i=0;i<max.size()-1;i++) {
		 * sb.append(max.get(i)+", "); } if (max.size()>0)
